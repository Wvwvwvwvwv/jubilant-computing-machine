...
def test_retrieval_worker_loop_processes_queued_jobs():
    ...
    async def _run():
        task = asyncio.create_task(retrieval_worker_loop(state, stop_event, pause_event))
...
def test_retrieval_worker_loop_honors_pause_event():
    state = RetrievalJobState()
    job = state.create_index_job(source_type="book", source_ref="paused_job")
    stop_event = asyncio.Event()
    pause_event = asyncio.Event()
    pause_event.set()
    ...
    asyncio.run(_run())
    assert state.get_job(job.job_id).status == "queued"
